...
parse_command_line (int argc, char *argv[], char *filename1, char *filename2,
					char *fileout, char *profileout, char *profilein, char *pairs_file)
{
	...
	if (argc > 2)
    {
		...
		for (i = 3; i < argc; i++)
		{
			...
			if (!strcmp (argv[i], "-profileout") && (argc > i + 1))
			{
				strcpy (profileout, argv[i + 1]);
				do_profile = 1;
				verbosity=0;
				norm_scores=1;
			} 
			
			if (!strcmp (argv[i], "-evd") && (argc > i + 1))
			{
				strcpy (profilein, argv[i + 1]);
				do_pvalues  = 1;
				norm_scores = 1;
			} 
			
			if (!strcmp (argv[i], "-evdloc") && (argc > i + 1))
			{
				evd_location=atof(argv[i + 1]);
				do_pvalues  = 1;
				norm_scores = 1;
			}   
			
			if (!strcmp (argv[i], "-evdscale") && (argc > i + 1))
			{
				evd_scale=atof(argv[i + 1]);
				do_pvalues  = 1;
				norm_scores = 1;
			} 
			...
			{
				p_threshold = strtod (argv[i + 1], &endptr);
			}
...
print_options ()
{
  ...
  printf (" -scale Z\tSet scaling parameter to Z\t\t[DEFAULT: %3.1lf]\n",
	  scale);
  printf (" -norm\t\tNormalise scores to a maximum of 100\t[DEFAULT: %s]\n",
          (char *) inttobool[norm_scores]);
  ...
  printf ("\nOther Options:\n");
  printf (" -profile\tChissa`?(Who knows?)\t\t\t[DEFAULT:]\n");
  printf (" -profileout\tChissa`?(Who knows?)\t\t\t[DEFAULT:]\n");
  printf (" -profile\tChissa`?(Who knows?)\t\t\t[DEFAULT:]\n");
  printf (" -evd\t\tUse Extreme Value distribution profile file\t\t\t[DEFAULT:]\n");
  printf (" -evdloc\tPass evd values\t\t\t[DEFAULT:]\n");
  printf (" -evdscale\tChissa`?\t\t\t[DEFAULT:]\n");
  printf (" -keyval\tKey value pairs ??\t\t\t[DEFAULT:]\n");
  printf (" -restrict\tChissa`?\t\t\t[DEFAULT:]\n");
...
printhit (char *query, char *reference, hit_struct * hit, char *sequence1,
		  char *sequence2, int direction, double z_score, double p_value, double energy,
		  FILE * fpout)
{
	...
	if (direction == FORWARD)
    {
		...
		(fpout,
		 ...
		 hit->score, 
		 (seqlen1_global-hit->query_end + 1), (seqlen1_global - hit->query_start +1), /*Doron - changed the reporting coordinates see Code_notes*/
	...
	fprintf (fpout, "\nScores for this hit:\n");
	fprintf (fpout,
			 ">%s\t%s\t%2.2lf\t%2.2lf\t%2.2lf\t%e\t%d %d\t%d %d\t%d\t%3.2lf%%\t%3.2lf%%\n\n",
			 query, reference, hit->score, energy, z_score, p_value,
			 (seqlen1_global - hit->query_end + 1), (seqlen1_global - hit->query_start + 1), /*Doron - changed coordinate - see code_notes*/
			 hit->ref_start + 1,
			 hit->ref_end + 1, alignment_length, identity, similarity);
}

void
printhit_tskv (char *query, char *reference, hit_struct * hit, char *sequence1,
			   char *sequence2, int direction, double z_score, double p_value, double energy,
			   FILE * fpout)
{
