...
void get_nt_nt_seq_scores (int **nt_nt_scores,char *sequence1, char *sequence2, int seqlen1, int seqlen2) {
	...
	if (seqlen1 < 3) exit(1); /* abort on miRNA string of length < 3 TODO: add input validity checking */
	for (j = 1; j <= seqlen2; j++) {
		...
		nt_nt_scores[seqlen1][j] = 0;
	}
	for (i = 3; i <= seqlen1 - 1; i++)  {
		...
		for (j = 1; j <= seqlen2; j++){
			...
			/* Debug Anton 
			if (i==j){
				printf("SCORE: %d %d %c %c->%d\n",i,j,sequence1[i-1],sequence2[i-1],nt_nt_scores[i][j]);
			}
			*/
...
int 
build_matrix_prof ( int **best, int ***track, int **a_nt_nt,int **b_gap_nt,int 
...
int best_score=0;
 ...
 for (i = 1; i <= seqlen1; i++) {
   for (j = 1; j <= seqlen2; j++) {
    ...
    if (i <= length_3p_for_weighting  || i == seqlen1){ 
        open=gap_open;ext=gap_extend;
    } else {
        if (!strict){
            open=scale*gap_open;ext=scale*gap_extend;
        } else {
            /* printf("Crippling Critical Region\n"); */
            open=-1000;
            ext=-1000; 
            if (nt_nt_score[i][j] < 0){
                nt_nt_score[i][j] = -1000;
            }
        }
    }
    ...
    best[i][j]=max_finder_and_track_threestates (a_nt_nt[i][j],b_gap_nt[i][j],c_nt_gap[i][j]);       
            ...
            if (best[i][j] > best_score){
                if ((i > (seqlen1-1)) || (!strict)){
                  best_score=best[i][j];
                  scores[1].score = best_score;
                  scores[1].path = 1;
                  scores[1].i = i;
                  scores[1].j = j;
                } else {
                    /* printf("Destroying Truncated Alignment\n"); */
                }
            }
    }
  }
...

{
	...
	for (i = 1; i <= seqlen1; i++) {
        ...
        return(good_count ? scores[0].score : -1);


} /* sub rooutine build_matrix_quick */
/*----------------------------------------------------------------*/
...
traceback (int **best, int ***track, char *sequence1, char *sequence2, int i,
		   int j, int remove, hit_struct * hit_ptr, double hit_score) {
