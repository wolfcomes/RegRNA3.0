...
python homer_findPeaksRAMPAGEbam.py /ReplaceRootPath //genomes//Databases//Drosophila//Drosophila_melanogaster//UCSC//dm6
...
python createBedFiles4ncRNAdowloadSeq.py /ReplaceRootPath //genomes//Databases//Drosophila RAMPAGE

# normalize the ElemeNT output so that every motif appears in a separate line
python normalizeElemeNToutputGen.py /ReplaceRootPath RAMPAGE

# prepare the input files for the R script that creates the graphs of elements position distribution and score
python prepMeanNpercent4graph-NascentAll.py /ReplaceRootPath RAMPAGE
