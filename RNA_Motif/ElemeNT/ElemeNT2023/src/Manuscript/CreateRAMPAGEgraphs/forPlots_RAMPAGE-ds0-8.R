...
{ 
  ...
  ggplot(df, aes(x = pos, y = percent, fill = MeanScore)) + 
    ...
    scale_y_continuous(breaks=c(10, 30)) + #manually specify y axis ticks: breaks=c(10, 30)
  ...
  sfn = paste(motif,"_",spname,"_",f_type,plot_sufix,sep="")
  ggsave(sfn,plot=last_plot(),height = 15, width = 10,dpi = 300) 
}
########################################################
plotgraphAll <- function(percent_fn, mean_fn, spname, f_type, motif, high_color, low_color ,plot_sufix, g_width, g_height)
{ 
  percent_fn = paste(lib_pref,spname,"_",f_type, "_",motif,"_percent.txt",sep="")
  percent <- read.table(percent_fn,header = TRUE)
  if (substr(motif,1,3) == "DPE") {
  ...
  ggplot(df, aes(x = pos, y = percent, fill = MeanScore)) + 
    ...
    scale_y_continuous(breaks=c(0, 5, 10, 15, 20, 25), expand = c(0, 0), limits = c(0, NA)) + #manually specify y axis ticks: breaks=c(10, 20, 30, 40, 50, 60)
    xlab(x_label) +  ylab("% of transcripts containing the element") +
    theme_classic()   +
    theme(axis.ticks.x = element_blank(),
          axis.title.x = element_text(vjust = -0.9,size = 33),
