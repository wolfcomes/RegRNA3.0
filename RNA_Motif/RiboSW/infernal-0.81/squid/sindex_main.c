 ...
 * 
 * SVN $Id: sindex_main.c 1739 2006-12-05 01:39:59Z eddys $
 */
...
static char experts[] = "\
  ...
  --pfamseq      : index a FASTA file with >(accession.version) (name) (desc)\n\
";
...
main(int argc, char **argv) 
{
  ...
  for (idx = optind; idx < argc; idx++)
	...
	/* In pfamseq, the FASTA format is 
	 *    ><accession>.<version> <name>  <free text description>
	 * and we want to be able to retrieve by accession alone.
	 * So: 
	 *   The sqinfo.name is parsed to obtain the accession, and
	 *   that's stored as a secondary key;
	 *   the first field of the descline is parsed to obtain
	 *   the actual name, and that's stored as a secondary key too.
	 */
	if (do_pfamseq && sfp->format == SQFILE_FASTA && sqinfo.desc != NULL) {
	  char *s, *nm;
	  
	  if (Strparse("(.+)\\.[0-9]+", sqinfo.name, 1))
	    {
	      status = SSIAddSecondaryKeyToIndex(ssi, sqd_parse[1], sqinfo.name);
	      if (status != 0) Die("SSIAddSecondaryKeyToIndex() failed");
	    }

	  s = sqinfo.desc;
	  nm = sre_strtok(&s, " \t", NULL);
	  if (nm != NULL) {
	    if ((status = SSIAddSecondaryKeyToIndex(ssi, nm, sqinfo.name)) != 0)
	    Die("SSI error: %s\n", SSIErrorString(status));
	  }
	}
