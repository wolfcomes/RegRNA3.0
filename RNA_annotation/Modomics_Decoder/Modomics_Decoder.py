...
options, args = getopt.getopt(argv, 
                              "i:atlhvf:", 
                              ["input=", "all", "table", "list", "help", "version", "fasta="])    
...
no_flag = not (input_flag or all_flag or table_flag or list_flag or help_flag or version_flag or fasta_flag)
...
FLAGS: (optional)
    ...
    -f --fasta:         Accepts a FASTA file as input instead of a direct sequence input.
...
# Handling FASTA input file
sequence = None
fasta_file = None
if fasta_flag:
    for flag, argument in options:
        if flag in ['-f', '--fasta']:
            fasta_file = argument  # Get the FASTA file path directly from the command-line argument
    ...
    try:
        with open(fasta_file, 'r') as file:
            for record in SeqIO.parse(file, "fasta"):
                ...
                break  # Only take the first sequence in the file
...
# If no sequence input, prompt for the RNA sequence
if not sequence:
    sequence = input('\nUNICODE Sequence: ')
    print('')
