...
def read_msa(filename: str, nseq: int) -> List[Tuple[str, str]]:
    ...
    return [(record.description,
           remove_insertions(str(record.seq).upper().replace('T', 'U'))) for record in itertools.islice(SeqIO.parse(filename, "fasta"), nseq)]

def read_fas(filename: str):
    """ Reads the first nseq sequences from an MSA file, automatically removes insertions."""
    return [(record.description,
             remove_insertions(str(record.seq).upper().replace('T', 'U'))) for record in itertools.islice(SeqIO.parse(filename, "fasta"), 1)]
